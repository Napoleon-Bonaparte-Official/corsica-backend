...
class UserAPI:
    class _CRUD(
        Resource
    ):  # User API operation for Create, Read.  THe Update, Delete methods need to be implemeented
        def post(self):  # Create method
            ...
            if dob is not None:
                try:
                    ...
                    return {
                        "message": f"Date of birth format error {dob}, must be mm-dd-yyyy"
                    }, 400
            ...
            return {
                "message": f"Processed {name}, either a format error or User ID {uid} is duplicate"
            }, 400
            ...
            try:
                ...
                if user:
                    try:
                        ...
                        resp = Response(
                            "Authentication for %s successful" % (user._uid)
                        )
                        resp.set_cookie(
                            "jwt",
                            token,
